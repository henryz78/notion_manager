...
func TestTranscriptJSONCarriesCapturedReasoningEffortForAllTurns(t *testing.T) {
	...
	full := buildFullTranscriptWithProtocol(
		account,
	...
	partial := buildPartialTranscriptWithProtocol(account, "continue", "strawberry-whoopiepie", "high", false, true, nil, false, nil, &Session{
		ConfigID:         "config-id",
...
func TestBuildPartialTranscriptUsesTwoContextsAndReusesContextPageID(t *testing.T) {
	...
	transcript := buildPartialTranscriptWithProtocol(account, "next question", "acai-budino-high", "", false, true, nil, false, nil, session, "")
	if len(transcript) != 6 {
	...
	buildPartialTranscriptWithProtocol(account, "another question", "acai-budino-high", "", false, true, nil, false, nil, session, "")
