...
最近 20 条批量任务记录保存在 `accounts/.account_batch_jobs.json`。Railway 休眠后重新启动时，已完成记录仍在；重启瞬间尚未结束的任务会标记为中断，未完成账号可通过“重试失败项”继续。添加账号也支持一行一个 token，前端默认同时验证 5 个。导入窗口可选择全部导入或只导入已设置个人指令的账号。
